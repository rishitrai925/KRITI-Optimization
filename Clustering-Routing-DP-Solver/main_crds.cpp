...
#include <fstream>
#include <filesystem>
#include "matrix.h" // Ensures we use your matrix loader and API
...
// Wrapper for matrix distance calls
double get_dist(const string& id1, const string& id2) {
    return getDistanceFromMatrix(id1, id2);
}
 ...
 */
struct Passenger {
    string id;            // Logic ID (e.g., "E1")
    string original_id;   // Matrix ID (e.g., "EMP001")
    int sequential_id;
...
struct Vehicle {
    string id;            // Logic ID (e.g., "V1")
    string original_id;   // Matrix ID (e.g., "KA01AB1234")
    int sequential_id;    // Fixed: Added this field
    int capacity;
    double available_time;
    string category;        
    double avg_speed;
    double cost_per_km;
    string current_loc_id; // Current location ID for matrix lookups
};

struct DPNode {
    string location_id; // ID for Matrix lookup
    double early;
    double late;
};
...
void loadMetadata(string filename) {
    ...
    while (getline(file, line)) {
        ...
        // Skip header
        if(first && normalize(cols[0]).find("parameter") != string::npos) { first = false; continue; }
...
class RoutingEngineCommonDrop {
    int N, NODES;
    vector<vector<double>> dp; // Stores WEIGHTED COST
    vector<vector<double>> time_state;
    vector<vector<int>> parent;
    vector<vector<double>> dist_state; // Tracks actual distance
    vector<DPNode> nodes;
    DPNode common_drop;
    ...
    double dist_lookup(int i, int j) {
        return get_dist(nodes[i].location_id, nodes[j].location_id);
    }
...
public:
    RouteResult calculate_optimal_route(Vehicle v, vector<Passenger>& cluster) {
        ...
        common_drop = {"OFFICE", 0, INF};
        
        double global_drop_deadline = INF;
        for(auto& p : cluster) global_drop_deadline = min(global_drop_deadline, p.latest_drop);

        nodes.resize(NODES);
        // Node 0 is Vehicle Start
        nodes[0] = {v.current_loc_id, v.available_time, INF};
        
        for(int i=0; i<N; i++) {
            nodes[i+1] = {cluster[i].original_id, cluster[i].earliest_pickup, cluster[i].latest_drop};
        }
        ...
        for (int i = 1; i < NODES; i++) {
            if (dp[finalMask][i] < INF) {
                // Calculate Drop Leg
                double dist_to_drop = get_dist(nodes[i].location_id, common_drop.location_id);
    ...
    pair<vector<pair<int, double>>, double> get_schedule(Vehicle v, vector<Passenger>& cluster) {
        ...
        DPNode drop = {"OFFICE", 0, INF};

        for (int i = 1; i < NODES; i++) {
             if (dp[finalMask][i] < INF) {
                double d = get_dist(nodes[i].location_id, drop.location_id);
        ...
        for(int node_idx : path) {
            double d = get_dist(nodes[prev_node_idx].location_id, nodes[node_idx].location_id);
        ...
        double d_drop = get_dist(nodes[prev_node_idx].location_id, drop.location_id);
...
double get_cost_single(Passenger p) {
    return get_dist(p.original_id, "OFFICE");
}

double get_batch_cost_common_drop(Passenger a, Passenger b) {
    double d1 = get_dist(a.original_id, b.original_id) + get_dist(b.original_id, "OFFICE");
    double d2 = get_dist(b.original_id, a.original_id) + get_dist(a.original_id, "OFFICE");
    return min(d1, d2);
}
...
vector<Passenger> loadPassengers(string filename) {
    ...
    while (getline(file, line)) {
        ...
        p.id = "E" + to_string(seq_id + 1); // Logic ID
        seq_id++;
...
vector<Vehicle> loadVehicles(string filename) {
    ...
    while (getline(file, line)) {
        ...
        v.current_loc_id = v.original_id; 
...
int main(int argc, char** argv) {
    ...
    // Load Matrix
    int N_PAS = passengers.size();
    int V_VEH = vehicles.size();
    int matrix_size = N_PAS + V_VEH + 1; // Employees + Vehicles + Office
    cout << "Loading matrix from " << matrix_path << " (Expecting ~" << matrix_size << " nodes)..." << endl;
    
    // Calls your matrix.h load function
    ...
    cout << "Calculating Dissimilarity Matrix..." << endl;
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
    ...
    cout << "Running Clustering Optimization..." << endl;
    while (true) {
        int target_idx = -1;
        double max_diam = -1.0;
    ...
    auto assign_group = [&](vector<Passenger>& grp) -> int {
        ...
        if (b_veh != -1) {
             ...
             cout << "  -> Weighted Score: " << b_res.weighted_score << endl;

             // Update vehicle location and availability
             assigned_v.current_loc_id = "OFFICE"; 
    ...
    cout << "===========================================" << endl;
    cout << "Output written to: " << outvehic_path << endl;
